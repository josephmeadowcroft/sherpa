...
function MainApp() {
  const { currentUser, userProfile, loading, updateUserProfile, refreshProfile } = useAuth();

  const [currentTab, setCurrentTab] = useState<'dashboard' | 'cv' | 'tracker'>('dashboard');
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      snapshot.docs.forEach((d) => items.push(d.data() as ApplicationRecord));
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      snapshot.docs.forEach((d) => items.push({ id: d.id, ...d.data() } as CvAnalysis));
      setCvAnalyses(items);
    });
    return () => unsubscribe();
  }, [currentUser]);
  ...
  const handleUploadCv = async (file: File) => {
    ...
    try {
      ...
      const runAnalysis = async (body: Record<string, any>) => {
        ...
        showToast('success', `CV Analyzed! Score: ${analysis.overallScore}/100`);
  ...
  const handleGenerateCv = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(result.error || 'Failed to generate updated CV.');

      setGeneratedCv(result);
      showToast('success', 'Updated CV generated! Download your PDF or LaTeX source below.');
    } catch (err: any) {
      console.error(err);
  ...
  const handleReanalyzeText = async () => {
    ...
    try {
      ...
      showToast('success', `Re-analyzed CV! New score: ${analysis.overallScore}/100`);
    } catch (err: any) {
      console.error(err);
  ...
  const needsOnboarding = !userProfile?.username || userProfile.onboardingCompleted === false;
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto p-4 sm:p-6 lg:p-8">
          ...
          <div className="max-w-5xl mx-auto space-y-6">
            <div className="flex items-center justify-between">
              <div>
                <h2 className="text-2xl font-bold text-gray-900 tracking-tight flex items-center gap-2">
                  <FileText className="w-6 h-6 text-blue-600" /> Resume & CV Optimizer
                </h2>
                <p className="text-xs text-gray-500 mt-1">
                  Instant ATS scoring, STAR bullet point rewrites, and tailored feedback.
                </p>
              </div>
              ...
              {userProfile?.cvText && (
                <div className="flex items-center gap-3">
                  ...
                  >
                    <RefreshCw className={`w-3.5 h-3.5 ${analyzingCv ? 'animate-spin text-blue-600' : ''}`} />
            ...
            {analyzingCv && (
              <div className="bg-white border border-gray-200/80 rounded-2xl p-12 text-center space-y-3 shadow-sm">
                ...
                <h3 className="text-sm font-bold text-gray-900">Analyzing Resume with Gemini AI...</h3>
                <p className="text-xs text-gray-500 max-w-sm mx-auto">
                  Extracting impact metrics, evaluating clarity, and crafting concrete STAR bullet point rewrites.
                </p>
              </div>
            )}
            ...
            {!latestCvAnalysis && !analyzingCv && (
              <div className="bg-white border-2 border-dashed border-gray-200 hover:border-blue-500 rounded-2xl p-12 text-center transition-colors shadow-2xs">
                <label className="cursor-pointer block space-y-4">
                  ...
                  <div>
                    <p className="text-base font-bold text-gray-900">Upload your PDF Resume</p>
                    <p className="text-xs text-gray-500 mt-1 max-w-sm mx-auto">
                      Drag & drop or click to upload your PDF CV. Gemini will score your resume and generate concrete rewritten examples.
                    </p>
                  </div>
                </label>
              </div>
        ...
        {currentTab === 'tracker' && (
          <div className="max-w-6xl mx-auto space-y-6">
            <div>
              <h2 className="text-2xl font-bold text-gray-900 tracking-tight flex items-center gap-2">
                <Compass className="w-6 h-6 text-blue-600" /> Internship & Graduate Application Tracker
              </h2>
              <p className="text-xs text-gray-500 mt-1">
                Real-time search grounded by Gemini AI — Track status across Saved, Applied, Interview, and Offer stages.
              </p>
            </div>
